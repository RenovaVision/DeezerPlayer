...
class TopArtistTracksJsonAdapter(
  moshi: Moshi
) : JsonAdapter<TopArtistTracks>() {
  private val options: JsonReader.Options = JsonReader.Options.of("data", "total", "prev", "next")

  private val listOfArtistTrackAdapter: JsonAdapter<List<TopArtistTracks.ArtistTrack>> =
      moshi.adapter(Types.newParameterizedType(List::class.java,
      TopArtistTracks.ArtistTrack::class.java), emptySet(), "data")
  ...
  override fun toString(): String = buildString(37) {
      append("GeneratedJsonAdapter(").append("TopArtistTracks").append(')') }

  override fun fromJson(reader: JsonReader): TopArtistTracks {
    var data: List<TopArtistTracks.ArtistTrack>? = null
    var total: Int? = null
    ...
    while (reader.hasNext()) {
      when (reader.selectName(options)) {
        0 -> data = listOfArtistTrackAdapter.fromJson(reader) ?: throw Util.unexpectedNull("data",
            "data", reader)
  ...
  override fun toJson(writer: JsonWriter, value: TopArtistTracks?) {
    if (value == null) {
      throw NullPointerException("value was null! Wrap in .nullSafe() to write nullable values.")
    }
    writer.beginObject()
    writer.name("data")
    listOfArtistTrackAdapter.toJson(writer, value.data)
    writer.name("total")
