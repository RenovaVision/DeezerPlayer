...
class ArtistFragment @Inject constructor(
    ...
    private val navArtistToAlbum: (id: @JvmSuppressWildcards Int) -> Unit,
    @Named("navArtistToPlayer")
    private val navArtistToPlayer: (track: @JvmSuppressWildcards PlayerModel) -> Unit
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val artist = arguments?.getSerializable("artist")?.let { it as ArtistEntity }
    ...
    private fun initViews(artist: ArtistEntity?) {
    ...
    override fun onStart() {
        ...
        viewModel.action.observe(this) {
            when (it) {
                is NavigateToAlbum -> navArtistToAlbum(it.id)
                is NavigateToPlayer -> navArtistToPlayer(it.track)
            }
        }
    }
}